...
const numSharpIterations =
  (Math.max(startingSharpQuality, 100 - startingSharpQuality) -
    sharpQualityFarPoint) /
    sharpQualityIncrement +
...
const watermarkFullResAndBlurFullResAndUpload = async (
  ...
  if (
    (await dontUploadFile(fullGCPFilePath)) &&
    (await dontUploadFile(blurGCPFilePath))
  ) {
    return;
  }
  ...
  if (
    !doingUploadExisting ||
    !fs.existsSync(fullLocalFilePath) ||
    !fs.existsSync(blurLocalFilePath)
  ) {
    ...
    await sharp(imageBuffer)
      ...
      .then(async (data) => {
        ...
        if (
          ...
          const increment =
            quality < 85 && quality > 15
              ? sharpQualityIncrement
              : sharpQualitySmallerIncrement;

          const newQuality =
            fileSizeKB < minFileSizeKB
              ? quality + increment
              : quality - increment;

          await optimizeAndUpload(
            origLocalFilePath,
            optimizedLocalFilePath,
            optimizedGCPFilePath,
            minFileSizeKB,
            maxFileSizeKB,
            newQuality,
            iteration + 1
...
const blurAndUploadAlbumPhoto = async (
  ...
  const albumBlurredLocalFilePath = albumLocalFilePath.replace(
    gcpAlbumPhotoEnding,
    gcpAlbumPhotoBlurredEnding
  );
  const albumBlurredGCPFilePath = albumGCPFilePath.replace(
    gcpAlbumPhotoEnding,
    gcpAlbumPhotoBlurredEnding
  );
  ...
  await Promise.all([
    ...
    sharp(watermarkMainLogoPathSquare)
      .resize({
        fit: sharp.fit.contain,
        width: Math.round(
          shortEdgeLength * watermarkWidthPercentOfShortEdgeSquare
        ),
      })
      .png({ quality: 100 })
...
const readDirAndUpload = async (path: string) => {
  ...
  for (const newCollectionFolder of newCollectionFolders) {
    ...
    if (fs.lstatSync(folderFilePath).isDirectory()) {
      ...
      const [
        fullLocalDirPath,
        optimizedLocalDirPathWeb,
        optimizedLocalDirPathMobile,
        optimizedLocalDirPathMobileGrid,
        blurLocalDirPath,
      ] = localDirPaths;
        ...
        try {
          if (!fs.lstatSync(filePath).isDirectory()) {
            if (newFile.includes(gcpAlbumPhotoEnding)) {
              ...
              const [
                fullGCPFilePath,
                optimizedGCPFilePathWeb,
                optimizedGCPFilePathMobile,
                optimizedGCPFilePathMobileGrid,
                blurGCPFilePath,
              ] = gcpFilePaths;
              ...
              await Promise.all([
                ...
                optimizeAndUpload(
                  filePath,
                  `${optimizedLocalDirPathWeb}/${newFile}`,
                  optimizedGCPFilePathWeb,
                  minFileSizeKBWeb,
                  maxFileSizeKBWeb
                ),
                ...
                optimizeAndUpload(
                  filePath,
                  `${optimizedLocalDirPathMobile}/${newFile}`,
                  optimizedGCPFilePathMobile,
                  minFileSizeKBMobile,
                  maxFileSizeKBMobile
                ),
                ...
                optimizeAndUpload(
                  filePath,
                  `${optimizedLocalDirPathMobileGrid}/${newFile}`,
                  optimizedGCPFilePathMobileGrid,
                  minFileSizeKBMobileGrid,
                  maxFileSizeKBMobileGrid
                ),
                // Full-Res and Blurred
                watermarkFullResAndBlurFullResAndUpload(
                  filePath,
                  `${fullLocalDirPath}/${newFile}`,
                  fullGCPFilePath,
                  `${blurLocalDirPath}/${newFile}`,
                  blurGCPFilePath
                ),
              ]);
            }
