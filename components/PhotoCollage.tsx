...
export default function PhotoCollage({
  ...
  const photoCollageUpdates = () => {
    if (
      window.matchMedia(
        `only screen and (min-width: 0px) and (max-width: ${maxWidthFullWidthCollections}px)`
      ).matches
    ) {
      if (widthVW !== 96) {
        setWidthVW(96);
  ...
  const getImage = (
    imgFolder: string,
    each: CollectionPhoto,
    webOrMobileDirName: string
  ) => {
    return (
      <Image
  ...
  return (
    ...
    >
      {collection.photos.map((each, index) => {
        const imgFolder =
          isHighlights && each.folder ? each.folder : collection.folder;
