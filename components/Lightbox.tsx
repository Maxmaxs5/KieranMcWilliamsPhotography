...
export default function Lightbox({
  ...
  if (
    currentIndexSharedState < 0 ||
    currentIndexSharedState >= collection.photos.length
  ) {
    return <></>;
  }
  ...
  const imgFolder =
    isHighlights && lightboxPhoto.folder
      ? lightboxPhoto.folder
      : collection.folder;

  const handleLightboxOrCloseClick: MouseEventHandler<HTMLDivElement> = (
    event
  ) => {
    setCurrentIndexSharedState(-1);
  };

  const handleLeftRightButtonClick = (
    ...
    setCurrentIndexSharedState(
      isLeftButton
        ? currentIndexSharedState <= 0
          ...
          : currentIndexSharedState - 1
        : currentIndexSharedState + 1 >= collection.photos.length
        ? 0
        : currentIndexSharedState + 1
    );
  };

  const handleClickFullResCheckbox: MouseEventHandler<HTMLDivElement> = (
    event
  ) => {
    cancelLightboxClose(event);
  ...
  const getFullResolutionDiv = () => {
    return (
      <>
        <div
          id="lightboxFullResolutionDiv"
          onClick={handleClickFullResCheckbox}
        >
          <input
            type="checkbox"
            name="fullResolution"
            checked={fullResolutionSharedState}
            readOnly
          />
  ...
  const getLeftRightButtonsAndName = () => {
    return (
      <>
        <h1
          id="lightboxLeftButton"
          onClick={(e) => handleLeftRightButtonClick(e, true)}
        >
          <FontAwesomeIcon icon={faAngleLeft} className="lightboxButtonIcon" />
        </h1>
        <h2 id="lightboxImageName">{lightboxPhoto.src}</h2>
        <h1
          id="lightboxRightButton"
          onClick={(e) => handleLeftRightButtonClick(e, false)}
        >
  ...
  const getCloseButton = () => {
    return (
      <h1 id="lightboxCloseButton" onClick={handleLightboxOrCloseClick}>
        Close
      </h1>
    );
  };
  ...
  const getImage = (webOrMobileDirName: string) => {
    return (
      <Image
        ...
        src={`${gcpBaseURL}/${imgFolder}/${
          fullResolutionSharedState ? gcpFullDirName : webOrMobileDirName
        }/${lightboxPhoto.src}`}
        unoptimized
        placeholder="blur"
  ...
  return (
    <div id="lightbox" onClick={handleLightboxOrCloseClick}>
      ...
      <div className="lightboxActionDiv" onClick={cancelLightboxClose}>
        ...
        <div
          className="verticalDivider alwaysLight"
          style={{ margin: "0 .25em 0 1em" }}
        />
      ...
      <div className="lightboxActionDiv mobile" onClick={cancelLightboxClose}>
        <div>{getLeftRightButtonsAndName()}</div>
